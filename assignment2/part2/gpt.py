...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        xq_odd = xq[..., 1::2]  # Shape: [batch, num_heads, seq_len, head_dim // 2]

        # Split key tensor into even and odd components
        xk_even = xk[..., ::2]  # Shape: [batch, num_heads, seq_len, head_dim // 2]
        xk_odd = xk[..., 1::2]  # Shape: [batch, num_heads, seq_len, head_dim // 2]

        # Apply RoPE transformation to query
        xq_rot_even = xq_even * pos_cos - xq_odd * pos_sin  # Even components
        xq_rot_odd = xq_even * pos_sin + xq_odd * pos_cos  # Odd components
        xq_rot = torch.stack([xq_rot_even, xq_rot_odd], dim=-1).reshape_as(xq)  # Combine and reshape

        # Apply RoPE transformation to key
        xk_rot_even = xk_even * pos_cos - xk_odd * pos_sin  # Even components
        xk_rot_odd = xk_even * pos_sin + xk_odd * pos_cos  # Odd components
        xk_rot = torch.stack([xk_rot_even, xk_rot_odd], dim=-1).reshape_as(xk)  # Combine and reshape
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        qkv = qkv.view(B, T, 3, self.n_head, self.n_embd // self.n_head)
        ...
        if self.use_flash_attn:
            y = F.scaled_dot_product_attention(q, k, v, None, self.attn_dropout.p, is_causal=True)

        else:
            # Compute attention scores
            attn = attn = torch.matmul(q, k.transpose(-2, -1)) * (self.n_embd // self.n_head) ** -0.5
            ...
            attn_probs = self.attn_dropout(attn_probs)    # Only during training???

            y = torch.matmul(attn_probs, v) # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        ...
        return y if not self.debug else {"att_probs": attn, "q": q, "k": k, "v": v}
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        att_input = self.layer_norm_1(x)
        att_output = self.self_attention(att_input)
        x = x + att_output  # Residual connection
        
        # Apply layer norm and MLP (with residual connection)
        mlp_input = self.layer_norm_2(x)
        mlp_output = self.mlpf(mlp_input)
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor):
        ...
        tok_emb = self.transformer.w_token_emb(idx)   # token embeddings of shape (b, t, n_embd)
